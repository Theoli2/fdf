...
int			mlx_ext_fullscreen(t_xvar *xvar, t_win_list *win, int fullscreen)
{
  ...
  while (i--)
	...
	if (res->modes[j].id == o_info->modes[i])
	  if (res->modes[j].height >= watt.height && res->modes[j].width >= watt.width &&
	      (idx_candidate == -1 || res->modes[idx_candidate].height > res->modes[j].height ||
  ...
  if (fullscreen)
    printf("found mode : %d x %d\n Status %d\n", res->modes[idx_candidate].height, res->modes[idx_candidate].width, i);
  else
