...
void		*mlx_int_xpm_f_image(t_xvar *xvar,int *height,int *width,
				     int (*xpm_func)(),void *param)
{
  ...
  if (!(im2 = (void *)mlx_new_image(xvar,img1->height,img1->width)))
    {
      XDestroyImage(img1);
      return ((void *)0);
    }
  ...
  *width = img1->width;
  if (mlx_int_egal_img(im2->image,img1))
    {
      bcopy(img1->data,im2->data,img1->width*img1->bytes_per_line);
      XDestroyImage(img1);
      return (im2);
    }
...
{
  return (mlx_int_xpm_f_image(xvar,height,width,XpmReadFileToImage,filename));
}


void	*mlx_xpm_to_image(t_xvar *xvar,char **data,int *height,int *width)
{
  return (mlx_int_xpm_f_image(xvar,height,width,XpmCreateImageFromData,(void *)data));
}
