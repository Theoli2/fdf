...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		if (!(line = f(info,&pos,info_size)) ||
						!(tab = mlx_int_str_to_wordtab(line)) || !(height = atoi(tab[0])) ||
		...
		while (i--)
		{
				...
				{
						if (!(clip_data = malloc(4*height*width)) ||   ok, nice size ..
										!(clip_img = XCreateImage(xvar->display, xvar->visual,
														1, XYPixmap, 0, clip_data,
														height, width, 8, (height+7)/8)) )
								RETURN;
						memset(clip_data, 0xFF, 4*height*width);
				}
				*/
				if (method)
		...
		if (!(img = mlx_new_image(xvar,height,width)))
				RETURN;
		opp = img->bpp/8;
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char *file,int *height,int *width)
{
		int	fd;
		int	size;
		char	*ptr;
...
void	*mlx_xpm_to_image(t_xvar *xvar,char **xpm_data,int *height,int *width)
{
		t_img	*img;
